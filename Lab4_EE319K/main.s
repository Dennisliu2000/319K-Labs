...
fin	  LDR	R0,=DataPt
	  ...
	  LDR	R1,[R0]
	  ORR	R1,#0x0FFFFFFF
	  STR	R1,[R0]	;makes minTime a very large positive number
	  
	  BL SysTick_Init
	  POP {R0-R4,PC}
	  BX LR
	  
;Debug capture has 44 lines, approx. 88 instruction cycles, requiring approx. 1100ns. 
;the program has approx. 3.2e6 lines to capture, approx. 6.4e6 inst cycles and approx. 80ms. 
;the overhead with an 80MHz clock is therefore 0.001375%
Debug_Capture 
      PUSH {R0-R10,LR}
	  LDR R7,=DataPt
	  ...
	  LDR R3,=maxTime
	  LDR R11,[R3]		;R11 HAS MAXIMUM TIME DIFF
	  CMP R1,R11
	  BLE min			;IF CURRENT VALUE<=MAX; SKIP STORING IT
	  STR R1,[R3]
min	  LDR R3,=minTime
	  LDR R11,[R3]		;R11 HAS MINIMUM TIME DIFF
	  CMP R1,R11
	  BGE skip			;IF CURRENT VALUE>=MIN; SKIP STORING IT
	  MOV R11,#0xFF
	  CMP R1,R11
	  BLE skip			;THIS CMP CHECKS TO SEE IF THE DIFF IS LESS THAN THE FIRST FAULTY VALUE-SKIPS IF THIS IS THE CASE
	  STR R1,[R3]
	  
skip  STR R1,[R5] ;store elapse to timebuffer
